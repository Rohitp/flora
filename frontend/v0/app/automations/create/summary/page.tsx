      ...
      {timelineSteps.map((step, i) => (
        <div key={step.day} className="flex gap-4">
          <div className="flex flex-col items-center">
            ...
            {i < timelineSteps.length - 1 && <div className="w-px flex-1 bg-[#e2e8f0]" />}
          </div>
          <div className="pb-8">
            <div className="flex items-baseline gap-2">
              <span className="text-xs font-semibold text-[#0f172a]">{step.day}</span>
              <span className="text-xs text-[#94a3b8]">{"\u2014"}</span>
              <span className="text-sm font-medium text-[#334155]">{step.label}</span>
            </div>
            <p className="mt-0.5 text-xs text-[#94a3b8]">{step.description}</p>
          </div>
        </div>
      ))}
    </div>
...
export default function CreateSummaryPage() {
  ...
  function handleActivate() {
    ...
    setTimeout(() => setPageState("success"), 2000)
    setTimeout(() => router.push("/automations"), 3800)
  }
    ...
    return (
      <div className="flex h-full items-center justify-center bg-white">
        <div className="flex flex-col items-center text-center animate-in fade-in duration-500">
          ...
          <p className="mt-2 text-sm text-[#94a3b8]">This will only take a moment.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex h-full items-center justify-center bg-white">
        <div className="flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-500">
          ...
          <p className="mt-2 text-sm text-[#94a3b8]">Redirecting to your automations...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex h-full flex-col bg-white">
      <header className="flex items-center border-b border-[#e2e8f0] px-6 py-4">
        <div className="flex items-center gap-3">
          <ForaAvatar />
          <div>
            <h1 className="text-sm font-semibold text-[#0f172a]">Review &amp; Activate</h1>
          ...
          <div className="mt-2 flex gap-6 rounded-lg border border-[#e2e8f0] bg-[#fafbfc] px-5 py-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#0f172a]">All customers</p>
              </div>
            </div>
          </div>
