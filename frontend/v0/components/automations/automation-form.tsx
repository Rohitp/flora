    ...
    <Card>
      <CardContent className="px-5 py-4">
        ...
        <div className="mt-5">
          {sorted.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border bg-muted/30 px-4 py-5 text-center">
              <p className="text-xs text-muted-foreground">No reminders configured yet.</p>
            </div>
          ) : (
...
function EmailTemplateDrawer({
  ...
  function handleSave() {
    if (!step) return
    onSave(step.id, { subject: localSubject, body: localBody, templateName: localName })
    onClose()
  }
            ...
            {showPreview ? (
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {["customer_name", "invoice_id", "invoice_amount", "due_date", "company_name", "payment_link"].map(
                    (v) => (
                      <button
                        key={v}
